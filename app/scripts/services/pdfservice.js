...
/**
 * @ngdoc service
 * @name dashboard20App.pdfService
 * @description
 * # pdfService
 * Service in the dashboard20App.
 */
angular.module('dashboard20App')
  .service('pdfService', function ($compile) {
    // AngularJS will instantiate a singleton by calling "new" on this function
	return {
			element		:	'',
			init		:	function( element , $scope ){
														
							this.element = element; //Set the Target Result element
							...
							angular.forEach(this.data.order.container,function(container,key){
								doc_order_table_keg_ordered += 	'<tr><td>'+container.size+' '+self.data.user_data.units+'</td>'+
							...
							/* Compile the document in the DOM */
							
							angular.element( self.element ).html($compile(doc_str)($scope));								
							//return doc_str;
						},
			document	:	new jsPDF('l', 'pt', 'a4'),
			generate	:	function(){
				...
				var result = angular.element( self.element );
				
				var specialElementHandlers = {
					pagesplit: true
				};
								
				self.document.text(35, 25, "This is a Doc 60").output("datauristring");
				
				this.document.addHTML(result.get(0), 15, 15, {
					//'width': 170, 
					'elementHandlers': specialElementHandlers
				},function(){
					//self.document.save(self.data.order.doc_type+'-'+self.data.order.ref+'.pdf');
					
					
					self.document.output("datauristring");
				});
